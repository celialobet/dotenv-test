...
client.search("#bonjour_monde",result_type: "recent", count: 100).collect do |tweet|
  if followers.include?(tweet.user.screen_name) == false
    followers << tweet.user.screen_name
  end
...
streaming.filter(track: "#bonjour_monde") do |tweet|
  client.follow(tweet.user)
  client.favorite(tweet)
  puts "Je viens de liker et follow"
end
